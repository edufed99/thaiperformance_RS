...
API_KEY = "sk-wkujsX0ljqnir_57UhF4Fg"   # ← ฝัง API key ตามที่ขอ (พิจารณาเก็บใน ENV สำหรับงานจริง)
MODEL_NAME = "deepseek/deepseek-r1-0528"  # ← ใช้ DeepSeek R1-0528

INPUT_CSV_FILE = "cluster_results.csv"
STOPWORDS_OUTPUT_CSV = "stopwords_analysis_DEEPSEEK.csv"
NON_STOPWORDS_OUTPUT_CSV = "non_stopwords_DEEPSEEK.csv"
PARTIAL_OUTPUT_CSV = "stopwords_analysis_DEEPSEEK.partial.csv"

# Runlog + metadata for resuming/retrying
RUNLOG_CSV = "stopwords_runlog.csv"
RUNMETA_JSON = "stopwords_runmeta.json"

REQUEST_TIMEOUT = 300        # DeepSeek R1 needs longer timeout (increased from 120)
TEMPERATURE = 0.1
MAX_TOKENS = 2048
CHUNK_SIZE = 20              # initial batch size
CHUNK_SIZE_RETRY = 12        # smaller size for failed-batch retries
MAX_RETRIES = 5
BACKOFF_BASE = 3.0           # Increased from 2.0 for longer waits
BACKOFF_JITTER = (0.5, 1.5)  # Larger jitter for timeout errors
BACKOFF_INITIAL = 5.0        # Initial wait before first retry
...
# ------------------------------------------------------------------------------
# DeepSeek response “reasoning” cleaner
# ------------------------------------------------------------------------------
RE_THINK_BLOCK = re.compile(r"<think>.*?</think>", re.DOTALL | re.IGNORECASE)
RE_REASONING_HDR = re.compile(r"^\s*(?:Reasoning|Thought|Thinking|Chain of Thought)\s*:.*?$", re.IGNORECASE | re.MULTILINE)
...
# ------------------------------------------------------------------------------
# API CALL WITH RETRIES (DeepSeek via ModelHarbor)
# ------------------------------------------------------------------------------
def call_api(prompt_content: str):
    session = get_request_session()
    headers = {"Authorization": f"Bearer {API_KEY}", "Content-Type": "application/json"}
    payload = {
        "model": MODEL_NAME,
        "messages": [
            {"role": "system", "content": "You are a helpful assistant. Respond ONLY in Thai. Do NOT include reasoning, thoughts, or chain-of-thought. Output must follow the required format."},
    ...
    for attempt in range(1, MAX_RETRIES + 1):
        try:
            ...
            print(f" ❌ หมดเวลา")
            print(f"   ⚠️  Attempt {attempt}/{MAX_RETRIES}: Timeout - รอ {wait_time:.1f}s ก่อนลองใหม่")
...
# ------------------------------------------------------------------------------
def process_span(span_df: pd.DataFrame, original_df: pd.DataFrame, sub_chunk_size: int = None):
...
# ------------------------------------------------------------------------------
def run_all():
    print("🚀 เริ่มกระบวนการจำแนก Stopword แบบ Batch (DeepSeek R1-0528 + Runlog + RetryFailed)")
...
def retry_failed_only():
    print("🔁 โหมดรันซ้ำเฉพาะ batch ที่ล้มเหลว")
...
# ------------------------------------------------------------------------------
def main():
    parser = argparse.ArgumentParser(description="Stopword batch analyzer with retry-failed support (DeepSeek R1-0528)")
