...
INPUT_CSV = "non_stopwords_gemini.csv"
OUTPUT_CSV = "classified_nonstopwords_gemini_output.csv"
...
def print_hierarchy(df: pd.DataFrame):
    if df.empty:
        ...
        return
    # สร้างโครงสร้างซ้อน: Main -> Sub -> Group -> Words(set)
    out = {}
...
if __name__ == "__main__":
    ...
    for i, chunk in enumerate(base_chunks, start=1):
        ...
        if df is None:
            ...
            continue
        # ตรวจภาษาไทยในสามคอลัมน์หลังพาร์สแต่ละชุด
